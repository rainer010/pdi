...
//        double inten_med = Core.mean(imagen).val[0];
    ...
    public static double getMSSIM(Mat i1, Mat i2) {
        ...
        Imgproc.GaussianBlur(I1, mu1, new Size(ventanaX, ventanaY), 1.5);
        I1.release();
        Imgproc.GaussianBlur(I2, mu2, new Size(ventanaX, ventanaY), 1.5);
        I2.release();
        //LIBERAR
        ...
        Imgproc.GaussianBlur(I1_2, sigma1_2, new Size(ventanaX, ventanaY), 1.5);
        I1_2.release();

        Core.subtract(sigma1_2, mu1_2, sigma1_2);
//        sigma1_2 -= mu1_2;

        Imgproc.GaussianBlur(I2_2, sigma2_2, new Size(ventanaX, ventanaY), 1.5);
        I2_2.release();
        Core.subtract(sigma2_2, mu2_2, sigma2_2);
//        sigma2_2 -= mu2_2;

        Imgproc.GaussianBlur(I1_I2, sigma12, new Size(ventanaX, ventanaY), 1.5);
        I1_I2.release();
...
//        return 1 - Math.abs((ambe) + (ssim ));
     ...
     * @return
     */
    public static Mat convertToMat(Integer[] vector, int filas, int columnas) {
...
//        System.out.print("---> "+contrasteG +"  "+contrasteS+" "+deltaC+"\n");
        if (deltaC > 0.01 ) {
            resultado =1-((ssim)*0.5 +(contrasteG/127.5)*0.5);
        }
        double[] r = new double[3];
        r[0] = resultado;
        r[1] = ssim;
        r[2] = contrasteG;
//        System.out.print("DEL ->"+deltaC+" "+(1-resultado)+"  "+ssim+"\n");
